...
Nix evaluates each shell into an immutable JSON manifest containing its
processes, environment, CLI scripts, tool paths, and tmux configuration. One
Rust executable embeds that manifest and selects its role from its executable
name or a private subcommand: controller, `dnvr` CLI, `dnvr-state`, sidebar,
log recorder, or process launcher. There is no generated controller script; a
shell is entered only for user-authored process commands, `prerun`, and
...
- Each process has both a raw `.log` transcript and an ANSI-stripped
  ...
  the appropriate file directly. tmux feeds a Rust recorder worker for each
  pane; the worker fans output into `rotatelogs` without a shell pipeline.
  Three 10 MiB files are retained per process and format.
...
Completions are generated by clap from the same typed command tree and
shell-specific manifest used by the CLI, so option parsing, help, and shell
completion cannot drift into separate implementations. `dnvr --list` remains
available as a tab-separated `command<TAB>description` interface for other
tools.
...
advisory lock on its `pid` file for life (the kernel drops the lock on
...
controller just opens the viewer, and another shell's running group is
never touched.
